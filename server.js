...
io.on("connection", (socket) => {
  ...
  socket.on("controlDevice", (data) => {
    // Dynamically construct topic: home/<room>/<device>/control
    const topic = `home/${data.room.replace(/\s+/g, "").toLowerCase()}/${data.device.replace(/\s+/g, "").toLowerCase()}/control`;
    mqttClient.publish(topic, data.command);
    console.log(`Published ${data.command} to ${topic}`);
  });
