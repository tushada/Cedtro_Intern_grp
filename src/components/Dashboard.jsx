...
const Dashboard = ({
  ...
  React.useEffect(() => {
    const ws = new window.WebSocket(
      "ws://" + window.location.hostname + ":1880/ws/env",
    );
  ...
  const handleDeviceToggle = async (room, deviceName) => {
    ...
    setRoomsData((prevRoomsData) => {
      ...
      const device = newRoomsData[room].devices.find(
        (d) => d.name === deviceName,
      );
    ...
    // Emit control event for any device dynamically
                    ...
                    <RoomSection>
                      ...
                      <Collapse in={selectedRoom === room}>
                        <Box sx={{ p: 2 }}>
                          ...
                          <Box sx={{ mt: 2 }}>
                            {(roomsData[room]?.devices || [])
                              .filter(
                                (device) => device.name !== "Only Smart TV",
                              )
                              .map((device, idx) => (
                                <DeviceCard
                                  key={device.name}
                                    ...
                                    device.name === "Smart TV" ? (
                                      <>
                                        <IconToggle
                                          ...
                                          onClick={() =>
                                            !isGuest &&
                                            handleSmartTVToggle(device.status)
                                          }
                                          sx={{ fontSize: "1.5rem" }}
                                        >
                                          {getDeviceEmoji(device.name)}
                                      ...
                                      <IconToggle
                                        ...
                                        onClick={() =>
                                          !isGuest &&
                                          handleDeviceToggle(room, device.name)
                                        }
                                        sx={{ fontSize: "1.5rem" }}
                                      >
                                        {getDeviceEmoji(device.name)}
