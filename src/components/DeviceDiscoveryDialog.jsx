...
export default function DeviceDiscoveryDialog({ open, onDone }) {
  ...
  const scanDevices = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        const discoveredDevices = response.data.devices.map(
          (device, index) => ({
            id: `dev-${index}`,
            name: device.name,
            type: device.service,
  ...
  return (
    ...
    >
      ...
      <DialogContent
        dividers
        sx={{ minHeight: 280, background: "rgba(30,30,30,0.95)" }}
      >
        {loading ? (
          <Box
            display="flex"
          ...
          >
            ...
            <Typography variant="body2" color="#bbb" sx={{ mt: 1 }}>
              Please make sure your devices are powered on and connected to the
              same Wi-Fi network.
              <br />
            ...
            {devices.map((device) => (
              <ListItem
                ...
                sx={{
                  borderRadius: 2,
                  mb: 1,
                  background: checked.includes(device.id)
